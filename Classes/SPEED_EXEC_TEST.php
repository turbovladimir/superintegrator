...
class SPEED_EXEC_TEST
{
  ...
  public function end_test()
  {
      if ($this->mode == 'print')
      {
          printf($this->script_name.' скрипт выполнялся %.4F сек.', $this->count_time());
      } elseif ($this->mode == 'log_file')
      {
          $fp = fopen('test_speed_log.txt', 'a');
          $log_date =  date("Y-m-d H:i:s");
          $string = sprintf("log:[%s] script_name:[%s] report:[%s] speed:[%F ���.]", $log_date, $this->script_name, $this->report, $this->count_time());
